...
def SetRatioCanvas(name, region, var):
    global logYAxis
    c1 = ROOT.TCanvas("%s_%s_%s" % (name, region, var), "%s_%s_%s" % (name, region, var), 0, 0, 800, 600)
    #c1.Divide(1,2,0,0)
...
def plotDeco(lumi, region, var, indir, debug):
    ...
    ROOT.gStyle.SetLegendBorderSize(0)
    ROOT.gROOT.ForceStyle()
    ROOT.myText(       0.21,  0.85, 1, "#sqrt{s}= 13 TeV, 10.0pb^{-1}")
    ...
    return True
#======================================================================
def plotCR(lumi, sample_path, region, variable, indir, debug):
    if debug: print "opening output file"
    o=ROOT.TFile("test.root", "RECREATE")
...
def plotSR(lumi, sample_path, region, variable, indir, debug):
    ...
    elif (var == 'AntiIDmuPt' or var == 'IDmuPt' ):
        sum1 = 0.0
        err1 = 0.0
        sum2 = 0.0
        err2 = 0.0
    ...
    for b in b_bins:
        ...
        if( args.doFF or args.doFF2D ):
            if( args.FFvar == 'eta' or args.FFvar == 'Eta' ):
                IDFF_var = 'IDmuEta'
                AIDFF_var = 'AntiIDmuEta'
            elif( args.FFvar == 'pt' or args.FFvar == 'Pt' ):
                IDFF_var = 'IDmuPt'
                AIDFF_var = 'AntiIDmuPt'
                ...
                print "Which FF var?"
                print "Not calculating fake factors and exiting!"
                sys.exit()

        if args.doFF:
            print 'Making 1D FFs and SR plots'
            ID_SR_path = 'FFIDSR/FFIDSR_FFIDSR%s/h_FFIDSR_FFIDSR%s_%s' % (b,b,IDFF_var)
            AID_SR_path = 'FFAIDSR/FFAIDSR_FFAIDSR%s/h_FFAIDSR_FFAIDSR%s_%s' % (b,b,AIDFF_var)
            num_list, num_error = plotSR(ID_lumi, ID_SR_path, 'ID'+b, IDFF_var,  args.indir, args.test)
            denom_list, denom_error = plotSR(AID_lumi, AID_SR_path, 'AID'+b, AIDFF_var,  args.indir, args.test)
            print 'Writing FFs'
            FFs, FF_err = divideAndErrors(num_list, num_error, denom_list, denom_error)
            if args.doCR:
                print 'Making 1D CR plots'
                for v in variables_ID:
                    ID_CR_path = 'FFIDSR/FFIDSR_FFID%s/h_FFIDSR_FFID%s_%s' % (b,b,v)
                    if args.test: print "running plot()"
                    plotCR(ID_lumi, ID_CR_path, 'ID'+b, v,  args.indir, args.test)
