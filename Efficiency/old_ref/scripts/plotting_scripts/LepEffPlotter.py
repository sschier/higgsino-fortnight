...
#======================================================================
def MakeLegend2(h_denom, t_denom, l_denom, h_num1, t_num1, l_num1, h_num2, t_num2, l_num2):
    ...
    l.AddEntry(h_num1, '%s_%s' % (t_num1, l_num1), 'l') 
    l.AddEntry(h_num2, '%s_%s' % (t_num2, l_num2), 'l') 
...
def MakeEff(topHist, bottomHist):
    ...
    rh.GetYaxis().SetTitleOffset(0.5)
    ...
    return rh
#======================================================================
def plot2(num1_path, num1_type, num1_lep,num2_path, num2_type, num2_lep, denom_path, denom_type, denom_lep, var, lepPair, indir, debug):
    ...
    if num1_type == 'truth':
        m_num1     = GetTruthHist(file_list, num1_path, debug)
    elif num1_type == 'reco':
        m_num1     = GetRecoHist(file_list, num1_path, debug)
    else: print "Please specify numerator type"
    if num2_type == 'truth':
        m_num2     = GetTruthHist(file_list, num2_path, debug)
    elif num2_type == 'reco':
        m_num2     = GetRecoHist(file_list, num2_path, debug)
    else: print "Please specify numerator type"
    m_eff1       = MakeEff(m_num1, m_denom)
    m_eff2       = MakeEff(m_num2, m_denom)
    #print "Making legend"
    m_legend = MakeLegend2(m_denom, denom_type, denom_lep, m_num1, num1_type, num1_lep, m_num2, num2_type, num2_lep,)
    ...
    m_num1.SetLineColor(43)
    m_num1.SetMarkerSize(0)
    m_num1.Scale(36000)
    m_num2.SetLineColor(13)
    ...
    m_eff1.SetLineColor(43)
    m_eff1.Draw("P")
    m_eff2.SetMarkerSize(0.1)
    m_eff2.SetLineColor(13)
    m_eff2.Draw("PSAME")
    p1.cd()
    m_denom.Draw("HIST")
    m_num1.Draw("HSAME")
    ...
    ROOT.myText(       0.21,  0.79, 1, 'm(#tilde{#Chi}_{2}^{0},#tilde{#Chi}_{1}^{0}) (%s, %s) GeV' % (N2, N1))
    #ROOT.myText(       0.41,  0.85, 1, "%s" % region)
    ROOT.ATLASLabel(0.21,0.91,"Internal")
    #raw_input("-->")
    canvas.Print('eff_%s_%s_%s_%s.pdf' % (lepPair, var, N2, N1))
    return True
#======================================================================
...
def main(argv):
    ...
    pair_list = ['EE', 'MM']
    var_list = ['Rll', 'Ptll', 'Mll', 'lepOrigin', 'lepType', 'lep1Origin', 'lep1Type', 'lep2Origin', 'lep2Type']
    for p in pair_list:
        tail_path_list = []
        base_path_list = []
        signal_path_list = []
        for v in var_list:
            ...
            tail_path_list  += ['baseLeptons%s/baseLeptons%s_tail/h_baseLeptons%s_tail_%s' % (p, p, p, v)]
        for s, b, t, v in zip(signal_path_list, base_path_list, tail_path_list, var_list):
            plot2(s, 'reco', 'signalLep', b, 'reco', 'baseLep', b, 'truth', 'baseLep', v, p, args.indir,  args.test)
            ...
            plot1(t, 'tail', 'baseLep', b, 'reco', 'baseLep', v, p, args.indir,  args.test)
            #plot1(t, 'tail', 'baseLep', t, 'truth', 'baseLep', v, p, args.indir,  args.test)
