...
#======================================================================
def mtautauValidation(event, hc, CutsDict):
    if CutsDict["MET200"] == False:
        return False
    ...
    if CutsDict["2SigLep"] == False:
        return False
    if CutsDict["dphiJMet2"] == False:
        return False
    if CutsDict["bVeto"] == False:
        return False

    hc.fill(event, "validation")
...
def analyze(infile, weightfile, tree, DSID, data, signal, outfile, debug, region):
    ...
    mtt=histcollection("mtt", o, debug, data, tree, DSID, sumWhist, 0)
    mtt.addvalidationcollection("validation")
    ...
    for event in t:
        ...
        if( jet1Vec.Pt() > 150. ):
            ...
            Cuts["jet1Pt150"] = False

        if( dphi_j1met > 2.0 ):
            Cuts["dphiJMet2"] = True
        else: Cuts["dphiJMet2"] = False
        ...
        #Perform selection 
        #------------------------------
        mtautauValidation(  event, mtt, Cuts)
